...
def build_attitude_nmpc_tv(model, N: int, weights=None, bounds=None, solver_opts=None):
    """NMPC with time-varying references over horizon.
    Parameters p = [x0(7); qref_stack(4*(N+1)); wref_stack(3*(N+1))].
    ...
    x0 = p[0:nx, :]
    off = nx
    qref_stack = p[off:off+4*(N+1), :]; off += 4*(N+1)
    wref_stack = p[off:off+3*(N+1), :]

    def qref_k(k):
        return qref_stack[4*k:4*(k+1), :]

    def wref_k(k):
        return wref_stack[3*k:3*(k+1), :]
    ...
    for k in range(N):
        ...
        att_err = attitude_cost_sign_invariant(qk, qref_k(k))
        J += w["wq"]*att_err + w["ww"]*ca.dot(wk - wref_k(k), wk - wref_k(k)) + w["wu"]*ca.dot(uk, uk)
        if k > 0:
    ...
    qN = X[0:4, N]
    wN = X[4:7, N]
    att_err_N = attitude_cost_sign_invariant(qN, qref_k(N))
    J += w["wqN"]*att_err_N + w["ww"]*ca.dot(wN - wref_k(N), wN - wref_k(N))
